...
# Execute build phase before running tests
& "$PSScriptRoot\build_all_impl.ps1" -TargetConfig $TargetConfig
if ($LASTEXITCODE -ne 0) {
    Write-Host "Error: Build phase failed with exit code $LASTEXITCODE - aborting tests." -ForegroundColor Red
...
foreach ($suite in $global:RUN_SUITES) {
    ...
    Write-Host "Running Tests: $configName" -ForegroundColor Cyan
    ...
    switch ($runnerType) {
        ...
        "node" {
            if ($global:NODE_BIN) {
                & $global:NODE_BIN "$fullBinaryPath" --test
                $exitCode = $LASTEXITCODE
            } else {
        ...
        "node_wasm_dir" {
            if ($global:NODE_BIN) {
                ...
                & $global:NODE_BIN "$binaryName" --test
                $exitCode = $LASTEXITCODE
                Pop-Location
            } else {
                Write-Host "Warning: Node.js not found, cannot run $binaryPath - skipping." -ForegroundColor Yellow
        ...
        "wasm" {
            Write-Host "Warning: WASM standalone test runner not supported on Windows native PowerShell - skipping." -ForegroundColor Yellow
            $skippedCount++
            continue
        }
        "none" {
    ...
    if ($exitCode -eq 0) {
        ...
        Write-Host "Error: Tests FAILED for $configName ($binaryPath) - exit code $exitCode" -ForegroundColor Red
        $failedSuites.Add("$configName ($binaryPath)")
        $failedCount++
    }
}
