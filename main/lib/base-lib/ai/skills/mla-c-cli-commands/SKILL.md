...
MyContext* ctx = mla_s_cast<MyContext*>(mla_platform_malloc(sizeof(MyContext)));
// … initialise ctx …
...
static mla_array_list_t<mla_init_struct(mla_string_t)> my_parameter_autocomplete(
    ...
    // Retrieve history candidates from application-managed store
    mla_array_list_t<mla_init_struct(mla_string_t)> candidates =
        mla_cli_history_get_candidates(app_history_store, mla_string_const("module:cmd:param"), currentValuePrefix);
